...
namespace cartographer_ros
{
        ...
        map_builder_bridge_(node_options_, std::move(map_builder), tf_buffer)
  {
    ...
    // Initialize ground truth tracking for relocalization error calculation
    ground_truth_initialized_ = false;

    // Get robot model name from ROS parameter, default to "robot"
    if (!node_handle_.getParam("robot_model_name", robot_model_name_))
    {
      robot_model_name_ = "turtlebot3_waffle"; // Default robot model name in Gazebo
    }

    // Subscribe to Gazebo model states for ground truth pose
    gazebo_model_states_subscriber_ = node_handle_.subscribe(
        "/gazebo/model_states", 1, &Node::HandleGazeboModelStatesMessage, this);

    std::cout << "Ground truth relocalization error monitoring initialized. "
                 "Robot model: "
              << robot_model_name_
              << " (set via ~robot_model_name parameter or default 'robot')" << std::endl;
  }
  ...
  void Node::PublishLocalTrajectoryData(const ::ros::TimerEvent &timer_event)
  {
    ...
    for (const auto &entry : map_builder_bridge_.GetLocalTrajectoryData())
    {
      ...
      // Calculate relocalization error using Gazebo ground truth
      static auto last_error_calc_time = std::chrono::steady_clock::now();
      static std::ofstream error_file; // 文件流对象

      auto current_time = std::chrono::steady_clock::now();
      auto error_time_diff = std::chrono::duration_cast<std::chrono::milliseconds>(
          current_time - last_error_calc_time);

      // 每5000ms计算一次误差
      bool calc_error = error_time_diff.count() >= 1000;

      if (calc_error && ground_truth_initialized_)
      {
        last_error_calc_time = current_time;

        const auto &reloc_translation = tracking_to_map.translation();
        const auto &reloc_rotation = tracking_to_map.rotation();

        // Convert relocalization quaternion to Euler angles (radians)
        double reloc_w = reloc_rotation.w();
        double reloc_x = reloc_rotation.x();
        double reloc_y = reloc_rotation.y();
        double reloc_z = reloc_rotation.z();

        double reloc_roll =
            std::atan2(2.0 * (reloc_w * reloc_x + reloc_y * reloc_z),
                       1.0 - 2.0 * (reloc_x * reloc_x + reloc_y * reloc_y));
        double reloc_sin_pitch = 2.0 * (reloc_w * reloc_y - reloc_z * reloc_x);
        double reloc_pitch = (std::abs(reloc_sin_pitch) >= 1
                                  ? std::copysign(M_PI / 2, reloc_sin_pitch)
                                  : std::asin(reloc_sin_pitch));
        double reloc_yaw =
            std::atan2(2.0 * (reloc_w * reloc_z + reloc_x * reloc_y),
                       1.0 - 2.0 * (reloc_y * reloc_y + reloc_z * reloc_z));

        const auto &gt_translation = ground_truth_pose_.translation();
        const auto &gt_rotation = ground_truth_pose_.rotation();

        // Convert ground truth quaternion to Euler angles (radians)
        double gt_w = gt_rotation.w();
        double gt_x = gt_rotation.x();
        double gt_y = gt_rotation.y();
        double gt_z = gt_rotation.z();

        double gt_roll = std::atan2(2.0 * (gt_w * gt_x + gt_y * gt_z),
                                    1.0 - 2.0 * (gt_x * gt_x + gt_y * gt_y));
        double gt_sin_pitch = 2.0 * (gt_w * gt_y - gt_z * gt_x);
        double gt_pitch =
            (std::abs(gt_sin_pitch) >= 1 ? std::copysign(M_PI / 2, gt_sin_pitch)
                                         : std::asin(gt_sin_pitch));
        double gt_yaw = std::atan2(2.0 * (gt_w * gt_z + gt_x * gt_y),
                                   1.0 - 2.0 * (gt_y * gt_y + gt_z * gt_z));

        // Calculate position and orientation errors
        double error_x = reloc_translation.x() - gt_translation.x();
        double error_y = reloc_translation.y() - gt_translation.y();
        double error_z = reloc_translation.z() - gt_translation.z();
        double error_roll = reloc_roll - gt_roll;
        double error_pitch = reloc_pitch - gt_pitch;
        double error_yaw = reloc_yaw - gt_yaw;

        // Handle angle wrapping for orientation errors (radians)
        while (error_roll > M_PI)
          error_roll -= 2.0 * M_PI;
        while (error_roll < -M_PI)
          error_roll += 2.0 * M_PI;
        while (error_pitch > M_PI)
          error_pitch -= 2.0 * M_PI;
        while (error_pitch < -M_PI)
          error_pitch += 2.0 * M_PI;
        ...
        double angular_error = error_yaw;

        // 创建错误信息字符串
        std::ostringstream error_msg;
        error_msg << "=== Relocalization Error Analysis (5s interval) ===" << std::endl;
        error_msg << "Relocalization ERROR:  dx=" << std::fixed
                  << std::setprecision(4) << error_x << " dy=" << error_y
                  << " dz=" << error_z << " (total=" << position_error
                  << "m) | "
                  << "droll=" << std::setprecision(4) << error_roll
                  << " dpitch=" << error_pitch << " dyaw=" << error_yaw
                  << " (total=" << angular_error << " rad)" << std::endl;
        ...
        {
          // 首次打开文件（追加模式）
          error_file.open("/home/s/error.txt", std::ios::app);
          if (!error_file.is_open())
          {
            LOG(ERROR) << "Failed to open error file: /home/s/error.txt";
          }
          else
          {
            LOG(INFO) << "Writing relocalization errors to /home/s/error.txt";
          }
        }
        ...
        if (error_file.is_open())
        {
          ...
          error_file << "[" << std::put_time(std::localtime(&now_time), "%Y-%m-%d %H:%M:%S")
                     << "] " << error_msg.str();
          error_file.flush(); // 确保立即写入
        }

        // Additional statistics (convert thresholds to radians for comparison)
        if (position_error > 0.1)
        { // Position error > 10cm
          LOG(WARNING) << "HIGH POSITION ERROR detected! Error: "
                       << position_error << "m";
        }
        if (angular_error > 0.087)
        { // Angular error > 5 degrees (0.087 radians)
          LOG(WARNING) << "HIGH ANGULAR ERROR detected! Error: "
                       << angular_error << " radians";
        }
      }
      else if (calc_error && !ground_truth_initialized_)
      {
        last_error_calc_time = current_time;
        LOG(WARNING)
            << "Gazebo ground truth not available yet. Check if "
               "/gazebo/model_states topic is published and robot model name '"
            << robot_model_name_ << "' exists in Gazebo.";
      }
  ...
  // Handle Gazebo model states for ground truth pose extraction
  void Node::HandleGazeboModelStatesMessage(
      const gazebo_msgs::ModelStates::ConstPtr &msg)
  {
    absl::MutexLock lock(&mutex_);

    // Find the robot model in the model states
    ...
    for (size_t i = 0; i < msg->name.size(); ++i)
    {
      if (msg->name[i] == robot_model_name_)
      {
        ...
        // Convert ROS geometry_msgs::Pose to Cartographer Rigid3d
        // 注意：可能需要坐标系转换，确保与Cartographer坐标系一致
        const Eigen::Vector3d translation(pose.position.x, pose.position.y,
                                          pose.position.z);
        const Eigen::Quaterniond rotation(pose.orientation.w, pose.orientation.x,
                                          pose.orientation.y, pose.orientation.z);

        // 如果需要坐标系转换，在这里添加
        // 例如：从Gazebo坐标系转换到map坐标系
        ground_truth_pose_ =
            cartographer::transform::Rigid3d(translation, rotation);

        if (!ground_truth_initialized_)
        {
          ground_truth_initialized_ = true;
          LOG(INFO) << "Gazebo ground truth tracking started for robot '"
                    << robot_model_name_ << "' at position: x=" << pose.position.x
    ...
    // If robot not found and this is the first time, log available models
    static bool warned_about_missing_robot = false;
    if (!robot_found && !warned_about_missing_robot)
    {
      std::stringstream available_models;
      for (size_t i = 0; i < msg->name.size(); ++i)
      {
        if (i > 0)
          available_models << ", ";
        available_models << "'" << msg->name[i] << "'";
      }
      LOG(WARNING)
          << "Robot model '" << robot_model_name_
          << "' not found in Gazebo. Available models: " << available_models.str()
          << ". Set correct model name using ~robot_model_name parameter.";
      warned_about_missing_robot = true;
    }
  }
