...
// void ResetInitPoseCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msg);

// // ----------------------------
// // 全局句柄（允许 RViz 重定位时调用）
// // ----------------------------
// cartographer_ros::Node* node_handle = nullptr;
// cartographer_ros::TrajectoryOptions* trajectory_options_handle = nullptr;

// // ----------------------------
// // gflags 参数
// // ----------------------------
// DEFINE_bool(collect_metrics, false, "Collect runtime metrics.");
// DEFINE_string(configuration_directory, "", "Directory of configuration files.");
// DEFINE_string(configuration_basename, "", "Basename of configuration file.");

// DEFINE_string(load_state_filename, "", "Path of pbstream to load.");
// DEFINE_bool(load_frozen_state, true, "Load pbstream as frozen trajectories.");

// DEFINE_bool(start_trajectory_with_default_topics, true,
//             "Start first trajectory automatically.");

// DEFINE_string(save_state_filename, "",
//               "If set, save pbstream before shutdown.");
...
//   // ------------------------------------------------------
//   // 参数：是否进入定位模式（localization 模式）
//   // ------------------------------------------------------
//   bool localization_mode = false;
//   pnh.param<bool>("/localization", localization_mode, false);

//   // 订阅 RViz “2D Pose Estimate” 用于重定位
//   ros::Subscriber init_pose_sub =
//       node.node_handle()->subscribe("/initialpose", 1, ResetInitPoseCallback);
...
//   // ------------------------------------------------------
//   // 非定位模式：自动启动轨迹
//   // 定位模式：等待 RViz /initialpose 手动启动
//   // ------------------------------------------------------
//   if (!localization_mode && FLAGS_start_trajectory_with_default_topics) {
//     node.StartTrajectoryWithDefaultTopics(trajectory_options);
//   }
...
// // ========================================================
// //        RViz 2D Pose Estimate 回调（重定位入口）
// // ========================================================
// void ResetInitPoseCallback(
...
//   // 设置初始位姿
//   *trajectory_options_handle
//        ->trajectory_builder_options
//        .mutable_initial_trajectory_pose()
//        ->mutable_relative_pose()
//       = cartographer::transform::ToProto(
//             cartographer_ros::ToRigid3d(msg->pose.pose));
...
//   // 订阅 RViz “2D Pose Estimate”（可选，不再强制需要）
//   // ros::Subscriber init_pose_sub =
//   //     node.node_handle()->subscribe("/initialpose", 1, ResetInitPoseCallback);
...
//   // ------------------------------------------------------
//   // 定位模式：自动设定初始位姿为 Identity（让 Cartographer 自动重定位）
//   // ------------------------------------------------------
...
//     *init_pose = cartographer::transform::ToProto(
...
//   // ------------------------------------------------------
//   // 关闭轨迹（定位模式不关闭，以保持 scan matcher 工作）
//   // ------------------------------------------------------
//   if (!localization_mode) {
//     node.FinishAllTrajectories();
//     node.RunFinalOptimization();
...
DEFINE_bool(start_trajectory_with_default_topics, true,
            "Start first trajectory automatically.");
DEFINE_string(save_state_filename, "", "If set, save pbstream before shutdown.");
...
void Run() {
  ...
  } else if (FLAGS_start_trajectory_with_default_topics) {
    node.StartTrajectoryWithDefaultTopics(trajectory_options);
  }

  ros::spin();

  // 定位模式下不关闭轨迹，保持 scan matcher 持续工作
  if (!localization_mode) {
    node.FinishAllTrajectories();
    node.RunFinalOptimization();
  }
