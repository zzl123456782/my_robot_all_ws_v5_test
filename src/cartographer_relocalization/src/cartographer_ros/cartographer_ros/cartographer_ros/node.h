...
class Node {
 ...
 private:
  ...
  // 重定位精度计算相关函数
  void HandleGazeboModelStatesMessage(
      const gazebo_msgs::ModelStates::ConstPtr& msg);
  void SetGroundTruthPose(const cartographer::transform::Rigid3d& pose);
  ...
  // Ground truth pose for relocalization error calculation
  ::ros::Subscriber gazebo_model_states_subscriber_;
  cartographer::transform::Rigid3d ground_truth_pose_ GUARDED_BY(mutex_);
  bool ground_truth_initialized_ GUARDED_BY(mutex_);
  std::string robot_model_name_;  // Name of robot model in Gazebo
