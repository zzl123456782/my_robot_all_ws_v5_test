 ...
 */
// double PoseGraph2D::ComputeConstraint(const NodeId& node_id,
//                                     const SubmapId& submap_id) {
...
//     //std::cout << "score is local finalllllllllllllllllllllllllllll" << constraint_builder_.GetLastConstraintScore()<<std::endl; 
//     return constraint_builder_.GetLastConstraintScore();   
...
//    // std::cout << "score is global finalllllllllllllllllllllllllllll" << constraint_builder_.GetLastConstraintScore()<<std::endl;
//     return constraint_builder_.GetLastConstraintScore();
...
double PoseGraph2D::ComputeConstraint(const NodeId& node_id,
...

  {
    ...
    if (!data_.submap_data.at(submap_id).submap->insertion_finished()) {
      // Uplink server only receives grids when they are finished, so skip
      // constraint search before that.
      std::cout << "子图未完成，跳过约束计算" << std::endl;
      return 0.0;
    }
    ...
    const double max_constraint_distance = 20.0; // 可配置的最大约束距离
    
    if (distance > max_constraint_distance) {
        //std::cout << "距离超过限制: " << distance << "m > " << max_constraint_distance 
                  //<< "m，跳过约束计算" << std::endl;
        return 0.0;
    }
    ...
    if (node_id.trajectory_id == submap_id.trajectory_id ||
      ...
      // 进行局部匹配
      maybe_add_local_constraint = true;
      //std::cout << "满足局部约束条件，距离: " << distance << "m" << std::endl;
    }
    // 如果两个不属于同一条轨迹 并且 最近也没有什么连接, 同时采样器为true
    // 才进行全局搜索, 也就是回环检测
    else if (global_localization_samplers_[node_id.trajectory_id]->Pulse()) {
      // 进行回环检测
      maybe_add_global_constraint = true;
      //std::cout << "满足全局约束条件（回环检测），距离: " << distance << "m" << std::endl;
    } else {
      // 重要：明确处理不满足任何约束条件的情况
      //std::cout << "不满足约束计算条件：不同轨迹且采样器未触发" << std::endl;
      return 0.0;
    }
  ...
  if (maybe_add_local_constraint) {
    ...
    const double local_score = constraint_builder_.GetLastConstraintScore();
    //std::cout << "局部约束计算完成，得分: " << local_score << std::endl;
    return local_score;   
    ...
    const double global_score = constraint_builder_.GetLastConstraintScore();
    //std::cout << "全局约束计算完成，得分: " << global_score << std::endl;
    return global_score;
  }
  
  // 这里不应该到达，但为了安全返回0.0
  //std::cout << "警告：约束计算逻辑异常，返回0.0" << std::endl;
  return 0.0;
}
...
//         return std::hash<int>()(id.trajectory_id) ^ (std::hash<int>()(id.submap_index) << 1);
//     }
// };
 
// // 修改后的主逻辑
// std::priority_queue<submap_sort> submap_sort_10;  // 10米内队列
// std::priority_queue<submap_sort> submap_sort_20;  // 10-20米队列
// std::unordered_set<SubmapId, SubmapIdHash> processed_submaps;  // 已处理子图记录
...
//     // 分阶段入队
//     if (distance <= 10.0) {
//         submap_sort_10.emplace(distance, submap_id.trajectory_id, submap_id.submap_index);
//     } else if (distance > 10.0 && distance <= 20.0) {
//         submap_sort_20.emplace(distance, submap_id.trajectory_id, submap_id.submap_index);
//     }
// }
...
// // 处理10米内子图（第一阶段）
// while (!submap_sort_10.empty()) {
//     const SubmapId id(submap_sort_10.top().trajectory_id, 
//                      submap_sort_10.top().index);
    
//     // 去重检查
//     if (processed_submaps.insert(id).second) {  // 插入成功说明未处理过
//     std::cout << "1111111111111111111111 " << submap_sort_10.size() << std::endl;
...
// // 处理10-20米子图（第二阶段）
// while (!submap_sort_20.empty()) {
//     const SubmapId id(submap_sort_20.top().trajectory_id,
//                      submap_sort_20.top().index);
    
//     // 去重检查
//     if (processed_submaps.insert(id).second) {
//     std::cout << "2222222222222222222222: " << submap_sort_10.size() << std::endl;
//         ComputeConstraint(node_id, id);
//     }
//     submap_sort_20.pop();
// }
...
//     }
// }
//   std::cout << "子图数量： "<< submap_sort_10.size() <<std::endl; 
//   std::cout << "子图数量d： "<< submap_sort_20.size() <<std::endl; 
//   // only use the nearist 1 or 2 or 3 constraint
// for (int i = 0; i < submap_sort_10.size(); i++) {

//   if (submap_sort_10.empty()) break;
  
//   SubmapId id(submap_sort_10.top().trajectory_id,submap_sort_10.top().index);
//   double score = ComputeConstraint(node_id, id);
//   std::cout << "zuihoude fenshu ssssssssssssssssssssssssss： "<< score <<std::endl; 
...
//     }
// };
// const double LOCAL_MATCH_SCORE_THRESHOLD = 0.5;   // 局部匹配得分阈值
// const double GLOBAL_MATCH_SCORE_THRESHOLD = 0.3;  // 全局匹配得分阈值（回环检测）
// const double MIN_VALID_SCORE = 0.0;                // 最小有效得分
...
//     std::cout << "20米内子图数量: " << submap_sort_10.size() 
//               << ", 20-40米子图数量: " << submap_sort_20.size() << std::endl;

//     bool relocalization_successful = false;

//     // 第一阶段：优先处理10米内的子图
//     std::cout << "开始第一阶段重定位（10米内）..." << std::endl;
//     while (!submap_sort_10.empty() && !relocalization_successful) {
//         const SubmapId id(submap_sort_10.top().trajectory_id,
//                           submap_sort_10.top().index);
//         const double distance = submap_sort_10.top().distance;

//         // 去重检查：确保不重复处理同一子图
//         if (processed_submaps.find(id) == processed_submaps.end()) {
//             processed_submaps.insert(id);
            
//             // 计算约束并获取匹配得分
//             const double match_score = ComputeConstraint(node_id, id);
            
//             // 检查是否计算成功
//             if (match_score > MIN_VALID_SCORE) {
//                 std::cout << "子图 [" << id.trajectory_id << "," << id.submap_index 
//                           << "] 距离: " << distance << "m, 匹配得分: " << match_score << std::endl;
                
//                 // 判断匹配类型和是否满足阈值
//                 bool is_relocalization_successful = false;
                
//                 // 根据轨迹ID判断是局部匹配还是全局匹配
//                 if (node_id.trajectory_id == id.trajectory_id) {
//                     // 同轨迹，局部匹配
//                     std::cout << "局部匹配（同轨迹）：";
//                     if (match_score >= LOCAL_MATCH_SCORE_THRESHOLD) {
//                         std::cout << "成功！得分 " << match_score 
//                                   << " >= 局部阈值 " << LOCAL_MATCH_SCORE_THRESHOLD << std::endl;
//                         is_relocalization_successful = true;
//                     } else {
//                         std::cout << "失败，得分 " << match_score 
//                                   << " < 局部阈值 " << LOCAL_MATCH_SCORE_THRESHOLD << std::endl;
//                     }
//                 } else {
//                     // 不同轨迹，全局匹配（回环检测）
//                     std::cout << "全局匹配（回环检测）：";
//                     if (match_score >= GLOBAL_MATCH_SCORE_THRESHOLD) {
//                         std::cout << "成功！得分 " << match_score 
//                                   << " >= 全局阈值 " << GLOBAL_MATCH_SCORE_THRESHOLD << std::endl;
//                         is_relocalization_successful = true;
//                     } else {
//                         std::cout << "失败，得分 " << match_score 
//                                   << " < 全局阈值 " << GLOBAL_MATCH_SCORE_THRESHOLD << std::endl;
...
//                 // 如果匹配成功，设置重定位成功并退出
//                 if (is_relocalization_successful) {
//                     std::cout << "重定位成功完成！" << std::endl;
//                     relocalization_successful = true;
//                     break; // 立即退出，避免不必要的计算
//                 } else {
//                     std::cout << "继续搜索其他候选子图..." << std::endl;
//                 }
//             }
//             else {
//                 std::cout << "子图 [" << id.trajectory_id << "," << id.submap_index 
//                           << "] 距离: " << distance << "m, 约束计算失败或得分无效（得分: " << match_score << "）" << std::endl;
//             }
//         }
...
//     // 第二阶段：如果10米内重定位失败，则在10-20米范围内尝试
//     if (!relocalization_successful && !submap_sort_20.empty()) {
//         std::cout << "第一阶段重定位失败，开始第二阶段重定位（10-20米）..." << std::endl;
        
//         while (!submap_sort_20.empty() && !relocalization_successful) {
//             const SubmapId id(submap_sort_20.top().trajectory_id,
//                               submap_sort_20.top().index);
//             const double distance = submap_sort_20.top().distance;

//             // 去重检查：确保不重复计算10米内已处理的子图
//             if (processed_submaps.find(id) == processed_submaps.end()) {
//                 processed_submaps.insert(id);
...
// // 输出最终重定位结果
// if (relocalization_successful) {
//     std::cout << "=== 重定位成功完成 ===" << std::endl;
//     LOG(INFO) << "Relocalization successful: local >= " << LOCAL_MATCH_SCORE_THRESHOLD 
//               << " AND global >= " << GLOBAL_MATCH_SCORE_THRESHOLD;
// }
// else {
//     std::cout << "=== 重定位失败，未找到同时满足局部和全局阈值的匹配 ===" << std::endl;
//     LOG(WARNING) << "Relocalization failed: no match found with both local >= " 
//                  << LOCAL_MATCH_SCORE_THRESHOLD << " and global >= " << GLOBAL_MATCH_SCORE_THRESHOLD;
// }
// // struct SubmapIdHash {
//     size_t operator()(const SubmapId& id) const {
//         return std::hash<int>()(id.trajectory_id) ^ (std::hash<int>()(id.submap_index) << 1);
...
// bool use_20_queue = false;  // 队列切换标志
// int processed_count = 0;    // 已处理计数器
 
// // 处理10米队列（未切换时）
// while (!submap_sort_10.empty() && !use_20_queue && processed_count < 3) {
//     const auto top = submap_sort_10.top();
//     submap_sort_10.pop();
    
//     SubmapId id(top.trajectory_id, top.index);
//     if (processed_submaps.count(id)) continue;
 
//     double score = ComputeConstraint(node_id, id);
//     std::cout << "当前得分： " << score << std::endl;
 
//     if (score < 0.5) {
//         use_20_queue = true;  // 触发队列切换
//         std::cout << "切换至10-20米队列" << std::endl;
//     } else {
//         processed_submaps.insert(id);
//         processed_count++;
//     }
...
// // 处理10-20米队列（切换后）
// if (use_20_queue) {
//     while (!submap_sort_20.empty() && processed_count < 3) {
//         const auto top = submap_sort_20.top();
//         submap_sort_20.pop();
...
//         processed_submaps.insert(id);
//         processed_count++;
//         std::cout << "处理10-20米子图，得分： " << score << std::endl;
//     }
// }
 
// // 清空剩余队列（可选）
// while (!submap_sort_10.empty()) submap_sort_10.pop();
// while (!submap_sort_20.empty()) submap_sort_20.pop();
...
//yuanban111111111111111111111
  ...
  //   if (initial_relative_pose.translation().norm() < 20.0) {
...
//     }
// };
// const double LOCAL_MATCH_SCORE_THRESHOLD = 0.4;   // 降低局部匹配得分阈值
// const double GLOBAL_MATCH_SCORE_THRESHOLD = 0.8;  // 降低全局匹配得分阈值
// const double MIN_VALID_SCORE = 0.0;                // 最小有效得分

// // 固定范围约束计算参数
// const double FIXED_RANGE_MIN = 0.0;               // 固定范围最小距离
// const double FIXED_RANGE_MAX = 20.0;              // 固定范围最大距离
// const int MAX_FIXED_RANGE_CANDIDATES = 10;         // 固定范围内最大候选数量

// // 分阶段队列：固定范围内的子图
// std::priority_queue<submap_sort> fixed_range_submaps;           // 固定范围内队列
// std::unordered_set<SubmapId, SubmapIdHash> processed_submaps;   // 已处理子图记录
    
// // 第一步：筛选固定范围内的子图进行约束计算
// for (const auto& submap_id : finished_submap_ids) {
//     // 计算节点到子图的相对位姿
//     const transform::Rigid2d initial_relative_pose =
//         optimization_problem_->submap_data()
...
//     // 只处理固定范围内的子图（FIXED_RANGE_MIN 到 FIXED_RANGE_MAX）
//     if (distance >= FIXED_RANGE_MIN && distance <= FIXED_RANGE_MAX) {
//         fixed_range_submaps.emplace(distance, submap_id.trajectory_id, submap_id.submap_index);
//         std::cout << "添加固定范围候选子图 [" << submap_id.trajectory_id << "," 
//                   << submap_id.submap_index << "] 距离: " << distance << "m" << std::endl;
//     }
// }

// std::cout << "固定范围内(" << FIXED_RANGE_MIN << "m-" << FIXED_RANGE_MAX 
//           << "m)子图数量: " << fixed_range_submaps.size() << std::endl;    bool relocalization_successful = false;

// // 固定范围约束计算：处理指定距离范围内的子图
// std::cout << "开始固定范围约束计算（" << FIXED_RANGE_MIN << "m-" << FIXED_RANGE_MAX 
//           << "m，最多处理" << MAX_FIXED_RANGE_CANDIDATES << "个候选）..." << std::endl;

// int fixed_range_processed = 0;
// while (!fixed_range_submaps.empty() && !relocalization_successful && 
//        fixed_range_processed < MAX_FIXED_RANGE_CANDIDATES) {
    
//     const SubmapId id(fixed_range_submaps.top().trajectory_id,
//                       fixed_range_submaps.top().index);
//     const double distance = fixed_range_submaps.top().distance;

//     // 去重检查：确保不重复处理同一子图
//     if (processed_submaps.find(id) == processed_submaps.end()) {
//         processed_submaps.insert(id);
//         fixed_range_processed++; // 增加处理计数
        
//         std::cout << "处理固定范围候选 " << fixed_range_processed << "/" 
//                   << MAX_FIXED_RANGE_CANDIDATES << " - 子图[" << id.trajectory_id 
//                   << "," << id.submap_index << "] 距离: " << distance << "m" << std::endl;
...
//         // 检查是否计算成功
//         if (match_score > MIN_VALID_SCORE) {
//             std::cout << "约束计算成功，匹配得分: " << match_score << std::endl;
...
//             // 如果匹配成功，设置重定位成功并退出
//             if (is_relocalization_successful) {
//                 std::cout << "固定范围约束计算成功完成！" << std::endl;
//                 relocalization_successful = true;
//                 break; // 立即退出，避免不必要的计算
//             } else {
//                 std::cout << "继续搜索其他候选子图..." << std::endl;
...
//         }
//         else {
//             std::cout << "约束计算失败或得分无效（得分: " << match_score << "）" << std::endl;
//         }
//     }
    
//     fixed_range_submaps.pop();
// }

// // 输出最终约束计算结果
// if (relocalization_successful) {
//     std::cout << "=== 固定范围约束计算成功完成 ===" << std::endl;
//     LOG(INFO) << "Fixed range constraint computation successful in range [" 
//               << FIXED_RANGE_MIN << "m, " << FIXED_RANGE_MAX << "m]";
// }
// else {
//     std::cout << "=== 固定范围约束计算失败，未找到满足阈值的匹配 ===" << std::endl;
//     LOG(WARNING) << "Fixed range constraint computation failed in range [" 
//                  << FIXED_RANGE_MIN << "m, " << FIXED_RANGE_MAX << "m]";
// }
...
const double LOCAL_MATCH_SCORE_THRESHOLD = 0.3;   // 局部匹配得分阈值
const double GLOBAL_MATCH_SCORE_THRESHOLD = 0.6;  // 全局匹配得分阈值
const double MIN_VALID_SCORE = 0.0;                // 最小有效得分

// 分阶段搜索参数
const double STAGE1_MIN = 0.0;                     // 第一阶段：0-10米
const double STAGE1_MAX = 10.0;
const double STAGE2_MIN = 10.0;                    // 第二阶段：10-20米
const double STAGE2_MAX = 20.0;
const int MAX_STAGE_CANDIDATES = 5;                // 每阶段最大候选数量

// 分阶段队列：按距离分类的子图
std::priority_queue<submap_sort> stage1_submaps;              // 0-10米队列
std::priority_queue<submap_sort> stage2_submaps;              // 10-20米队列
std::vector<SubmapId> global_candidates;                      // 全局匹配候选
std::unordered_set<SubmapId, SubmapIdHash> processed_submaps; // 已处理子图记录
...
for (const auto& submap_id : finished_submap_ids) {
    ...
    // 按距离分类子图
    if (distance >= STAGE1_MIN && distance < STAGE1_MAX) {
        // 第一阶段：0-10米
        stage1_submaps.emplace(distance, submap_id.trajectory_id, submap_id.submap_index);
       // std::cout << "添加第一阶段候选子图 [" << submap_id.trajectory_id << "," 
                  //<< submap_id.submap_index << "] 距离: " << distance << "m" << std::endl;
    } else if (distance >= STAGE2_MIN && distance < STAGE2_MAX) {
        // 第二阶段：10-20米
        stage2_submaps.emplace(distance, submap_id.trajectory_id, submap_id.submap_index);
        //std::cout << "添加第二阶段候选子图 [" << submap_id.trajectory_id << "," 
                 // << submap_id.submap_index << "] 距离: " << distance << "m" << std::endl;
    } else {
        // 全局匹配候选：超过20米的所有子图
        global_candidates.push_back(submap_id);
       // std::cout << "添加全局匹配候选子图 [" << submap_id.trajectory_id << "," 
                //  << submap_id.submap_index << "] 距离: " << distance << "m" << std::endl;
    }
}

//std::cout << "分阶段统计：" << std::endl;
//std::cout << "  第一阶段(0-10m): " << stage1_submaps.size() << " 个子图" << std::endl;
//std::cout << "  第二阶段(10-20m): " << stage2_submaps.size() << " 个子图" << std::endl;
//std::cout << "  全局匹配(>20m): " << global_candidates.size() << " 个子图" << std::endl;
bool relocalization_successful = false;

// =================== 第一阶段：0-10米范围搜索 ===================
//std::cout << "\n=== 开始第一阶段搜索 (0-10米) ===" << std::endl;
int stage1_processed = 0;
while (!stage1_submaps.empty() && !relocalization_successful && 
       stage1_processed < MAX_STAGE_CANDIDATES) {
    
    const SubmapId id(stage1_submaps.top().trajectory_id,
                      stage1_submaps.top().index);
    const double distance = stage1_submaps.top().distance;
    ...
    if (processed_submaps.find(id) == processed_submaps.end()) {
        ...
        //std::cout << "处理第一阶段候选 " << stage1_processed << "/" 
                 // << MAX_STAGE_CANDIDATES << " - 子图[" << id.trajectory_id 
                 // << "," << id.submap_index << "] 距离: " << distance << "m" << std::endl;
        ...
        // 检查是否满足阈值
        if (match_score > MIN_VALID_SCORE) {
            bool is_match_successful = false;
            
            if (node_id.trajectory_id == id.trajectory_id) {
                // 局部匹配
                if (match_score >= LOCAL_MATCH_SCORE_THRESHOLD) {
                   // std::cout << "✅ 第一阶段局部匹配成功！得分: " << match_score << std::endl;
                    is_match_successful = true;
                }
            } else {
                // 全局匹配
                if (match_score >= GLOBAL_MATCH_SCORE_THRESHOLD) {
                    //std::cout << "✅ 第一阶段全局匹配成功！得分: " << match_score << std::endl;
                    is_match_successful = true;
                }
            }
            ...
            if (is_match_successful) {
                relocalization_successful = true;
                //std::cout << "第一阶段搜索成功，停止后续搜索！" << std::endl;
                break;
            }
        }
    }
...
// =================== 第二阶段：10-20米范围搜索 ===================
if (!relocalization_successful) {
    //std::cout << "\n=== 第一阶段未找到匹配，开始第二阶段搜索 (10-20米) ===" << std::endl;
    int stage2_processed = 0;
    while (!stage2_submaps.empty() && !relocalization_successful && 
           stage2_processed < MAX_STAGE_CANDIDATES) {
        
        const SubmapId id(stage2_submaps.top().trajectory_id,
                          stage2_submaps.top().index);
        const double distance = stage2_submaps.top().distance;
        ...
        if (processed_submaps.find(id) == processed_submaps.end()) {
            ...
            //std::cout << "处理第二阶段候选 " << stage2_processed << "/" 
                     // << MAX_STAGE_CANDIDATES << " - 子图[" << id.trajectory_id 
                    //  << "," << id.submap_index << "] 距离: " << distance << "m" << std::endl;
            ...
            if (match_score > MIN_VALID_SCORE) {
                ...
                if (node_id.trajectory_id == id.trajectory_id) {
                    // 局部匹配
                    if (match_score >= LOCAL_MATCH_SCORE_THRESHOLD) {
                        //std::cout << "✅ 第二阶段局部匹配成功！得分: " << match_score << std::endl;
                        is_match_successful = true;
                    }
                } else {
                    // 全局匹配
                    if (match_score >= GLOBAL_MATCH_SCORE_THRESHOLD) {
                       // std::cout << "✅ 第二阶段全局匹配成功！得分: " << match_score << std::endl;
                        is_match_successful = true;
                    }
                }
                ...
                if (is_match_successful) {
                    relocalization_successful = true;
                    //std::cout << "第二阶段搜索成功，停止后续搜索！" << std::endl;
                    break;
                }
            }
        }
...
// =================== 第三阶段：全局匹配搜索 ===================
if (!relocalization_successful) {
    //std::cout << "\n=== 前两阶段未找到匹配，开始全局匹配搜索 (>20米) ===" << std::endl;
    int global_processed = 0;
    for (const auto& submap_id : global_candidates) {
        if (relocalization_successful || global_processed >= MAX_STAGE_CANDIDATES) break;
        
        // 去重检查
        if (processed_submaps.find(submap_id) == processed_submaps.end()) {
            processed_submaps.insert(submap_id);
            global_processed++;
            ...
            //std::cout << "处理全局匹配候选 " << global_processed << "/" 
                      //<< MAX_STAGE_CANDIDATES << " - 子图[" << submap_id.trajectory_id 
                      //<< "," << submap_id.submap_index << "] 距离: " << distance << "m" << std::endl;

            // 计算约束并获取匹配得分
            const double match_score = ComputeConstraint(node_id, submap_id);
            
            // 全局匹配使用较低阈值
            if (match_score >= GLOBAL_MATCH_SCORE_THRESHOLD) {
                //std::cout << "✅ 全局匹配成功！得分: " << match_score << std::endl;
                relocalization_successful = true;
                break;
            }
        }
...
if (relocalization_successful) {
    //std::cout << "\n=== 分阶段重定位成功完成 ===" << std::endl;
    //LOG(INFO) << "Staged relocalization successful";
} else {
    //std::cout << "\n=== 分阶段重定位失败，所有阶段均未找到满足阈值的匹配 ===" << std::endl;
    //LOG(WARNING) << "Staged relocalization failed - no matches found in all stages";
}
