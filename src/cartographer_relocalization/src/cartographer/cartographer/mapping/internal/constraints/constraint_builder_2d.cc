...
void ConstraintBuilder2D::ComputeConstraint(
  ...
  if (match_full_submap) {
    ...
    if (submap_scan_matcher.fast_correlative_scan_matcher->MatchFullSubmap(
      ...
      {
        absl::MutexLock locker(&score_mutex_);
        last_constraint_score_ = static_cast<double>(score);
        std::cout << "global score succeed issssssssssss" << last_constraint_score_<<std::endl;
        //std::cout << "global________lasttttttttttttttttttttttttttt" << last_constraint_score_<<std::endl;
    ...
    } else {
      {
        absl::MutexLock locker(&score_mutex_);
        last_constraint_score_ = static_cast<double>(score);
        std::cout << "global score falied issssssssssss" << last_constraint_score_<<std::endl;
      }
      return;
    }
  } 
    ...
    if (submap_scan_matcher.fast_correlative_scan_matcher->Match(
      ...
      kConstraintScoresMetric->Observe(score);
      {
        absl::MutexLock locker(&score_mutex_);
        last_constraint_score_ = static_cast<double>(score);
        std::cout << "local__________lasttttttttttttttttttttttttttt" << last_constraint_score_<<std::endl;
      }      
    ...
    } else {
     {
        absl::MutexLock locker(&score_mutex_);
        last_constraint_score_ = static_cast<double>(score);
        std::cout << "local score falied issssssssssss" << last_constraint_score_<<std::endl;
     }
...
double ConstraintBuilder2D::GetLastConstraintScore() const {
  absl::MutexLock locker(&score_mutex_);
  return last_constraint_score_;
}
