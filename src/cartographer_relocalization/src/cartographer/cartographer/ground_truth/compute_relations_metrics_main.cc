...
void Run(const std::string& pose_graph_filename,
  ...
  if (!errors.empty()) {
    ...
    for (const Error& error : errors) {
      avg_trans_error += std::sqrt(error.translational_squared);
      avg_rot_error += common::RadToDeg(std::sqrt(error.rotational_squared));
    }
    
    avg_trans_error /= errors.size();
    avg_rot_error /= errors.size();
    
    std::cout << "平均平移误差: " << avg_trans_error << " m" << std::endl;
    std::cout << "平均旋转误差: " << avg_rot_error << " deg" << std::endl;
  }
}
